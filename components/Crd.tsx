...
const CBody= ({content}:{content:string}) =>{
    return <p className='mx-1 p-2 text-white line-clamp-5 md:line-clamp-none '>{content}</p>
}
function CFooter({linker}:{linker:string}){
    return <a className='border text-blue-500 px-2 py-1 rounded-lg max-w-max m-3  ' href="/contact"> {linker} </a>
}
